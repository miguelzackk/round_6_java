...
public class Jogo extends Jogador {
	...
	// Métodos para jogar como guarda
	public void jogarQuadrado(Scanner input) {
		...
		for (int i = 1; i <= 6; i++) {
			...
			// Atualiza a lista
			quadrado01.setListaJogadores(1, quadrado01.
				getListaJogadores(1) - this.getEliminacoesDia(i));
			quadrado01.setListaJogadores(0, quadrado01.
				getListaJogadores(0) + this.getEliminacoesDia(i));

			if (quadrado01.getEstaVivo()) {
				do {
					System.out.printf("\nDia: %d | Jogo: %s\n", i,
						this.getNomeJogo(i));
					System.out.printf("\nHoje é o %dº dia, o que você vai "
						+ "fazer? \n", i);
					...
					switch (escolhaMenu) {
						...
						case 2:
							...
							break;
						default:
							System.out.printf("'%d' não é uma escolha "
								+ "possível.\n", escolhaMenu);
							break;
					}
				} while(!escolhaValida);
				...
				if (quadrado01.getEstaVivo() && (i == 2 || i == 4)) {
					do {
						System.out.println(quadrado01.escolherTeste(
							escolhaTeste));
						...
						switch (escolhaMenu) {
							case 1:
								...
								System.out.println("Você foi visto quebrando "
									+ "as regras e executado.");
								break;
							case 2:
		...
		if (quadrado01.getEstaVivo()) {
			...
			System.out.println("Bom trabalho! Esteja preparado para retornar "
				+ "em breve!");
		}
	}
		...
		for (int i = 1; i <= 6; i++) {
			...
			if (triangulo01.getEstaVivo()) {
				...
				if (triangulo01.getEstaVivo() && (i == 2 || i == 4)) {
					do {
						System.out.println(triangulo01.escolherTeste(
							escolhaTeste));
						System.out.println("| 1 | - Sim\n"
							+ "| 2 | - Não");
						escolhaMenu = input.nextInt();
		...
		for (int i = 1; i <= 6; i++) {
			...
			if (circulo01.getEstaVivo()) {
				...
				if (circulo01.getEstaVivo() && (i == 2 || i == 4)) {
					do {
						System.out.println(circulo01.escolherTeste(
							escolhaTeste));
						System.out.println("| 1 | - Sim\n"
							+ "| 2 | - Não");
						escolhaMenu = input.nextInt();
